...
class _InversionScreenState extends State<InversionScreen> {
  ...
  Future<void> _cargarPrecioOro() async {
    try {
      final String raw = await DefaultAssetBundle.of(context)
          .loadString('lib/assets/gold_price_data.json');
      final List data = json.decode(raw);
      final today = DateTime.now().toIso8601String().split('T')[0];
      final item =
          data.firstWhere((e) => e['date'] == today, orElse: () => data.last);
  ...
  Future<void> _cargarInversiones() async {
    ...
    for (final inv in todas) {
      final service = InversionService();
      final ganancia = await service.calcularGananciaPorcentualDesdeOro(
          inv.cantidad, inv.fechaInicio);
        ...
        child: Column(
          ...
          children: [
            ...
            TextField(
              ...
              decoration:
                  const InputDecoration(labelText: "Cantidad a invertir en €"),
            ),
            ...
            ElevatedButton(
              ...
              onPressed: () async {
                ...
                if (cantidad != null) {
                  final valido =
                      await InversionValidationService.validarSaldoDisponible(
                          userId, cantidad);
                  if (!valido) {
                  ...
                  final confirm =
                      await _confirmar("¿Deseas invertir €$cantidad?");
                  if (confirm) {
  ...
  Widget _buildCard(InversionModel inv) {
    ...
    final Color cardColor =
        isActiva ? Colors.amber.shade50 : Colors.grey.shade100;
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
              ...
              children: [
                ...
                if (isActiva)
                  IconButton(
                    ...
                    onPressed: () async {
                      final confirm =
                          await _confirmar("¿Finalizar esta inversión?");
                      if (confirm) {
            ...
            Text(
                'Rendimiento acumulado: ${inv.rendimiento.toStringAsFixed(2)}%',
                style: const TextStyle(fontSize: 14)),
            if (ganancia != null)
              Text(
                  '📈 Evolución del oro: ${ganancia >= 0 ? '+' : ''}${ganancia.toStringAsFixed(2)}%',
            ...
            Row(
              ...
              children: [
                Text(
                    'Inicio: ${inv.fechaInicio.toLocal().toString().split(" ")[0]}'),
                Text('Estado: ${inv.estado}')
              ],
            ),
            if (inv.fechaFin != null)
              Text('Fin: ${inv.fechaFin!.toLocal().toString().split(" ")[0]}',
  ...
  Widget _buildLista(String titulo, List<InversionModel> lista, bool expandido,
      Function toggle) {
    return Card(
      ...
      child: ExpansionTile(
        ...
        children: lista.isEmpty
            ? [
                const Padding(
                    padding: EdgeInsets.all(8), child: Text("Sin inversiones."))
              ]
            : lista.map(_buildCard).toList(),
      ),
    );
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [
          IconButton(
              onPressed: _crearNuevaInversion,
      ...
      body: _loading
          ...
          : ListView(
              children: [
                _buildLista("Activas", _activas, _mostrarActivas,
                    () => setState(() => _mostrarActivas = !_mostrarActivas)),
                _buildLista(
                    "Completadas",
                    _completadas,
                    _mostrarCompletadas,
